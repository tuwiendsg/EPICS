...
public class DataElasticityMonitor implements Runnable {
    ...
    public void startMonitoringService() {
        ...
        if (dataAssetIndex.equals(latestDataAssetIndex)) {
            System.out.println("ORCHESTRATOR: WAITING FOR LATEST DATA ASSET WINDOW");
        } else {
            ...
            iou1.writeData("START - session: " + monitoringSession.getSessionID() +" - "
                    + "window: " + monitoringSession.getDataAssetIndex() +"\n",
                    "execution_engine.txt");
            
            System.out.println("ORCHESTRATOR session " + monitoringSession.getSessionID() + " : LATEST DATA ASSET WINDOW: " + dataAssetIndex);

            List<MonitoringAction> listOfMonitoringActions = monitorProcess.getListOfMonitoringActions();

            Logger.logInfo("Execute Monitoring Process ... " + monitoringSession.getSessionID());
            ...
            Logger.logInfo(monitoringSession.getSessionID() + " MONITORING RESULT: " + monitoringSession.getSessionID() + " \n");
            ...
            for (MonitoringMetric monitoringMetric : listOfMonitoringMetrics) {
                logMessage += "\t" + monitoringMetric.getMetricValue() + "\t100";
            ...
            iou.writeData(logMessage, "edaas_quality_" + monitoringSession.getSessionID() + ".txt");
...
//            controller.startControlElasticState(currentElasticState);
                ...
                ProcessExecutor processExecutor = new ProcessExecutor(listOfElasticStates, listOfAdjustmentProcess, monitoringSession, currentElasticState, dataAssetIndex, t1);
