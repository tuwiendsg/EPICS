...
public class SALSAConnector {
	...
	static final String DEPIC_SERVICE_NAME = "depic";
	static final String DEPIC_TOPOLOGY_NAME = "depicSimpleTopology";

	List<DeployAction> deployActions;
	
	// this DEPIC endpoint point to a single the ServiceUnit. By adding action name behind, it is the deply
	String salsaDEPICEndpoint = "http://128.130.172.215:8080/salsa-engine/rest/services/" + DEPIC_SERVICE_NAME + "/topologies/" + DEPIC_TOPOLOGY_NAME + "/nodes/";
	String salsaRESTfulEndpoint = "http://128.130.172.215:8080/salsa-engine/rest";
	...
	private void updateListWithDeploymentEndpoint(){
		for (DeployAction action : this.deployActions) {
			action.setDeploymentEndpoint(salsaDEPICEndpoint + action.actionID+ "/instance-count/1");
		}
	}
	...
	public String toToscaString() {
		...
		for (DeployAction action : this.deployActions) {		
			...
			artTemplate.setId(action.actionID + "_artifact");
			...
			artRef.setReference(action.artifact);
