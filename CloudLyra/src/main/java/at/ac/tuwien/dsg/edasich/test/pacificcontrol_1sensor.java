...
import static at.ac.tuwien.dsg.edasich.test.pacificcontrol_3sensors.inTable;
import static at.ac.tuwien.dsg.edasich.test.pacificcontrol_3sensors.machine;
 ...
 * @author Jun
 */
public class pacificcontrol_1sensor {
    ...
    public static void main(String[] args) throws JAXBException {
        ...
        String dafName = "space_temperature_analytic";
    ...
    public static DafAnalyticCep sampleAnalytic(){
       ...
       // String epl = "select * from SensorEvent.win:time(30 sec) match_recognize ( measures A as val1, B as val2 pattern (A+ B+) define A as A.value < 13 and A.name ='chw_supply_temp',  B as B.value > 24 and B.name ='fcu_ff1_space_temp')";
        String epl = "select * from SensorEvent match_recognize ( "
                  + "measures A as val1 "
                  + "pattern (A)"//interval 1 seconds "
                  + "define A as (Math.round(A.value) = 23 and A.name ='fcu_ff1_space_temp"+ machine+"')  "
                  + ")";
    ...
    public static AnalyticResultDelegate sampleAnalyticResultDelegate(){
       ...
       // String delegateMessage="<task><taskID>id01</taskID><taskName>CLEAN EVAPORATOR.</taskName><taskContent>Evaporator is fouling.</taskContent><tag>IMPLEMENTATION</tag><severity>WARNING</severity></task>";
        ...
        Task delegateMessage = new Task(1, "Evaporator is foulding potential","Turn on sensors, which monitor supply temperature and fcu set point for futher data analytics. ", "EVAPORATOR", Task.SeverityLevel.NOTICE);
